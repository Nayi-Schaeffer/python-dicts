import time 

#En el contexto de la programación las funciones son una secuencia de sentencias de código con un nombre.
#Se parecen en algo a las variables en el sentido que son un simple nombre. En el caso de las variables, estas apuntan a un valor, mientras que en el caso de las funciones apuntan a una serie de sentencias que realizan una tarea específica.

#Python trae varias funciones ya listas como las clásicas print(), int(), len(, etc) 

#Podemos definir nuevas funciones para especificar un nombre junto con una secuencia de sentencias que se ejecutan cuando la función es "llamada", "ejecutada" o "invocada"

#Unas funciones pueden requerir argumentos Ej: print() recibe como argumento lo que va a imprimir en la terminal. Algunas funciones reciben más de un argumento.

#Un detalle es que cuando estamos dentro de la función los argumentos sea signan a variables locales llamadas **parámetros** 

#def palabra clave para definir una nueva función (def = Defifir)

verses = ["Mano hacia alfrente", "Puño cerrado", "Dedo hacia arriba", "lengua afuera"]


def intro(verse):#Argumento
    print(f"Atención, compañía {verse}")#Parámetro
    time.sleep(1)

def chorus():
    print("Chuchuwa")
    time.sleep(1)
    print("Chuchuwa")
    time.sleep(1)
    print("Chuchuwa")
    time.sleep(1)
    print("Chuchu wa wa wa")
    time.sleep(1)

def outro():
    print("Lala")
    time.sleep(1)
    print("Lala")
    time.sleep(1)
    print("lala  la la la  ")
    print("Lala")
    print("Lala")
    time.sleep(1)
    print("lala la la la  ")

for verse in verses:
    intro(verse)
    #Esto revisa si el verso No es el ultimo de los versos

    if verse != verses[-1]:
        chorus()
    else:
        outro()
    print("--------------")

